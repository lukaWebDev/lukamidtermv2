...
const students = [
  ...
  function getMultiAchivers(students, threshold) {
    ...
    function getMultiAchivers(students, threshold) {
        return students
          .map(student => {
            const highGradeSubjects = Object.values(student.grades).filter(grade => grade > threshold).length;
            return { name: `${student.firstName} ${student.lastName}`, expertSubjects: highGradeSubjects };
          })
          .filter(student => student.expertSubjects >= 2);
      }
  }
